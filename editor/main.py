...
class Editor(QMainWindow):
    def __init__(self,project:str|Path):
        super().__init__(); self.project=Path(project); self.current=None; self.setWindowTitle(f'PyNovel Editor 0.5 — {self.project.name}'); self.resize(1480,900); self.status=QStatusBar(); self.setStatusBar(self.status)
        ...
        self.scene_tab=SceneEditor(self.project); self.tabs.addTab(self.scene_tab,'Scene'); self.dialogue_tab=DialogueEditor(self.project); self.tabs.addTab(self.dialogue_tab,'Dialogue'); self.reload()
    ...
    def open_folder(self):
        ...
        if path:self.project=Path(path); self.current=None; self.setWindowTitle(f'PyNovel Editor 0.5 — {self.project.name}'); self.reload()
def main():
