...
class Editor(QMainWindow):
    def __init__(self,project:str|Path):
        super().__init__(); self.project=Path(project); self.current=None; self.setWindowTitle(f'PyNovel Editor 0.21 — {self.project.name}'); self.resize(1600,920); self.status=QStatusBar(); self.setStatusBar(self.status)
        ...
        self.scene_tab=SceneEditor(self.project); self.tabs.addTab(self.scene_tab,'Scene'); self.dialogue_tab=DialogueEditor(self.project); self.tabs.addTab(self.dialogue_tab,'Dialogue'); self.ui_tab=UIEditor(self.project); self.tabs.addTab(self.ui_tab,'UI'); self.asset_tab=AssetBrowser(self.project); self.tabs.addTab(self.asset_tab,'Assets'); self.reload()
    ...
    def open_folder(self):
        ...
        if path:self.project=Path(path); self.current=None; self.reload()
def main():
