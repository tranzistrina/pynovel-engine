...
from editor.timeline_editor import TimelineEditor
from vnengine.script.parser import VNParser,VNParseError
ASSET_EXTS={'.png','.jpg','.jpeg','.webp','.wav','.ogg','.mp3','.mp4','.json','.vn'}
class Editor(QMainWindow):
    def __init__(self,project:str|Path):
        super().__init__(); self.project=Path(project); self.current=None; self.setWindowTitle(f'PyNovel Editor 0.24 — {self.project.name}'); self.resize(1600,920); self.status=QStatusBar(); self.setStatusBar(self.status)
        ...
        self.scene_tab=SceneEditor(self.project); self.tabs.addTab(self.scene_tab,'Scene'); self.dialogue_tab=DialogueEditor(self.project); self.tabs.addTab(self.dialogue_tab,'Dialogue'); self.ui_tab=UIEditor(self.project); self.tabs.addTab(self.ui_tab,'UI'); self.timeline_tab=TimelineEditor(self.project); self.tabs.addTab(self.timeline_tab,'Animation'); self.ui_asset_drop=UIAssetDropBridge(self.ui_tab); self.asset_tab=AssetBrowser(self.project); self.tabs.addTab(self.asset_tab,'Assets'); self.reload()
    ...
    def save_file(self):
        ...
        if current is self.ui_tab:self.ui_tab.save_file(); self.status.showMessage('UI saved'); return
        if current is self.timeline_tab:self.timeline_tab.save_file(); self.status.showMessage('Animation saved'); return
