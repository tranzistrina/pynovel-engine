...
class UIEditor(QWidget):
    def __init__(self, project: str | Path):
        super().__init__()
        self.project = Path(project); self.path = self.project / "ui.json"
        self.current = None; self.items = {}; self.labels = {}; self.history = []; self.future = []; self._restoring = False
        self._drag_start = None; self._drag_origin = None; self._resize_start = None; self._resize_origin = None
        ...
        left = QWidget(); ll = QVBoxLayout(left); ll.addWidget(QLabel("Widgets"))
        self.tree = QTreeWidget(); self.tree.setHeaderLabels(["ID", "Type"]); self.tree.itemClicked.connect(self.select_tree); ll.addWidget(self.tree)
        addrow = QHBoxLayout(); add = QComboBox(); add.addItems(WIDGET_TYPES); addrow.addWidget(add)
        btn = QPushButton("Add"); btn.clicked.connect(lambda: self.add_widget(add.currentText())); addrow.addWidget(btn); ll.addLayout(addrow); split.addWidget(left)
        self.scene = QGraphicsScene(0, 0, 1280, 720); self.scene.setBackgroundBrush(QBrush(QColor(35, 38, 48)))
        self.view = QGraphicsView(self.scene); self.view.setSceneRect(0, 0, 1280, 720); self.view.setMouseTracking(True); self.view.viewport().installEventFilter(self); split.addWidget(self.view)
        right = QWidget(); rl = QVBoxLayout(right); rl.addWidget(QLabel("Inspector")); form = QFormLayout()
        self.id_edit = QLineEdit(); self.type_box = QComboBox(); self.type_box.addItems(WIDGET_TYPES); self.x = QSpinBox(); self.x.setRange(-5000,5000); self.y = QSpinBox(); self.y.setRange(-5000,5000)
        self.w = QSpinBox(); self.w.setRange(1,5000); self.h = QSpinBox(); self.h.setRange(1,5000); self.text = QLineEdit(); self.anchor = QComboBox(); self.anchor.addItems(ANCHORS); self.z = QSpinBox(); self.z.setRange(-9999,9999)
        ...
        rl.addLayout(actions); save = QPushButton("Save UI"); save.clicked.connect(self.save_file); rl.addWidget(save); rl.addStretch(); split.addWidget(right); split.setSizes([260,780,320]); self.load_file()
        ...
        if watched is self.view.viewport() and event.type() == event.MouseButtonPress:
            ...
            if item is not None:
                ...
                if isinstance(data, dict):
                    self.current = data; self.load_inspector(data); self._record(); self._drag_start = pos; self._drag_origin = (self.resolve(data.get('x',0),1280), self.resolve(data.get('y',0),720)); return True
            ...
            self.current['x'] = int(self._drag_origin[0] + delta.x()); self.current['y'] = int(self._drag_origin[1] + delta.y()); self.rebuild(preserve_current=True); self.load_inspector(self.current); return True
        elif watched is self.view.viewport() and event.type() == event.MouseButtonRelease:
            self._drag_start = None; self._drag_origin = None; return False
    ...
    def rebuild(self, preserve_current=False):
        ...
        self.tree.clear(); self.scene.clear(); self.items = {}; self.labels = {}; 
        self.populate_tree(self.tree.invisibleRootItem(), self.data); self.draw_widget(self.data)
        if current_id:
            found = self.find_by_id(current_id, self.data); self.current = found
            if found: self.load_inspector(found)
    ...
    def geom(self, data, parent_offset=(0,0)):
        x = self.resolve(data.get('x',0),1280) + parent_offset[0]; y = self.resolve(data.get('y',0),720) + parent_offset[1]
        w = self.resolve(data.get('width',120),1280); h = self.resolve(data.get('height',40),720); a = data.get('anchor','top-left')
    ...
    def delete_current(self):
        ...
        if parent:
            self._record(); parent['children'].remove(self.current); self.current = None; self.rebuild()
    def duplicate_current(self):
        if self.current is None: return
        parent = self.find_parent(self.current) or self.data; self._record(); clone = copy.deepcopy(self.current); base = str(clone.get('id','widget')); ids = {n.get('id') for n in self.iter_nodes(self.data)}; i=1; new_id=f'{base}_copy';
