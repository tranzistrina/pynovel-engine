...
def test_map_world_end_to_end_move_and_restore_state():
    ...
    world.selection.select("army_1"); world.selection.select("army_2", additive=True)
    command = MovementCommand(RouteBuilder(definition), world.movement)
    result = command.execute(world.selection, "c", speed=100)
    assert result if False else len(result) == 2
    world.update(0.5)
