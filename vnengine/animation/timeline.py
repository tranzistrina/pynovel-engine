...
class Track:
    ...
    def add(self, key: Keyframe) -> None:
    ...
    def sample(self, time: float) -> float | None:
        ...
        for left, right in zip(self.keys, self.keys[1:]):
            if left.time <= time <= right.time:
                ...
                progress = ease(progress, left.easing)
...
class Timeline:
    ...
    def add_track(self, track: Track) -> Track:
        ...
        self.duration = max(self.duration, track.duration)
        return track
    ...
    def add_keyframe(self, target: str, property: str, time: float, value: float, easing: str = "linear") -> None:
        ...
        if track is None:
            track = self.add_track(Track(target, property))
        track.add(Keyframe(time, value, easing))
        self.duration = max(self.duration, time)

    def sample(self, time: float | None = None) -> dict[tuple[str, str], float]:
        current = self.time if time is None else time
        return {
            (track.target, track.property): value
            for track in self.tracks
            if (value := track.sample(current)) is not None
        }
    ...
    def seek(self, time: float) -> None:
        if self.duration <= 0:
            ...
            return
        if self.loop:
            self.time = max(0.0, time) % self.duration
        else:
            self.time = max(0.0, min(self.duration, time))
    ...
    def update(self, dt: float) -> dict[tuple[str, str], float]:
        if self.playing:
            self.seek(self.time + max(0.0, dt))
        return self.sample()
            ...
            "tracks": [
                {
                    ...
                    "keys": [
                        {"time": key.time, "value": key.value, "easing": key.easing}
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "Timeline":
        ...
        for raw_track in payload.get("tracks", []):
            ...
            for raw_key in raw_track.get("keys", []):
                track.add(
                    Keyframe(
                        float(raw_key.get("time", 0)),
                        float(raw_key.get("value", 0)),
                        str(raw_key.get("easing", "linear")),
                    )
                )
            timeline.add_track(track)
        timeline.duration = max(timeline.duration, float(payload.get("duration", timeline.duration)))
        return timeline
