...
class DeclarativeScene:
    ...
    def __init__(self, definition: dict[str, Any], runtime: Any):
        ...
        self.logic: GameLogic = getattr(runtime, "logic", None) or GameLogic()
        if getattr(runtime, "logic", None) is None:
            runtime.logic = self.logic
        self.assets = getattr(runtime, "assets", None)
    ...
    def refresh(self) -> None:
        ...
        while 0 <= self.index < len(self.actions) and guard <= len(self.actions):
            ...
            if kind == "label": self.index += 1
                ...
                branch = action.get("then", []) if self._condition(action.get("condition")) else action.get("else", [])
    ...
    def _condition(self, condition: Any) -> bool:
        if condition is None: return True
        evaluator = getattr(self.runtime, "evaluate", None)
        if callable(evaluator): return bool(evaluator(condition))
        if isinstance(condition, dict): return self.logic.check(condition)
    ...
    def _goto(self, action: dict[str, Any]) -> None:
        ...
        if target in self.labels:
            ...
            return
        switch_scene = getattr(self.runtime, "switch_scene", None)
        if callable(switch_scene):
            scenes = getattr(self.runtime, "scenes", None)
            ids = getattr(scenes, "ids", None)
            if not callable(ids) or target in ids():
                switch_scene(target)
                return
    ...
    def _choices(self) -> list[tuple[int, dict[str, Any]]]:
        if not (0 <= self.index < len(self.actions)): return []
        if self.actions[self.index].get("type") != "choice": return []
        result = []
        cursor = self.index
        while cursor < len(self.actions):
            action = self.actions[cursor]
            if not isinstance(action, dict) or action.get("type") != "choice": break
            if self._condition(action.get("condition")): result.append((cursor, action))
            cursor += 1
        return result
    ...
    def _choose(self, number: int) -> bool:
        ...
        action_index, choice = choices[number]
        self.logic.events.append({"type": "choice", "index": number, "action_index": action_index, "text": choice.get("text", "")})
    ...
    def handle_input(self, event: Any) -> bool:
        ...
        if event.type == self.pygame.MOUSEBUTTONDOWN and event.button == 1:
            choices = self._choices()
            return self._choose(self.selected_choice) if choices else self._advance()
        return False
    ...
    def render(self, target: Any) -> None:
        ...
        target.fill(tuple(self.definition.get("background_color", (24, 24, 32))))
        image = self._load_background()
        if image is not None:
            size = image.get_size(); scale = min(width / max(1, size[0]), height / max(1, size[1]))
            image = self.pygame.transform.smoothscale(image, (max(1, int(size[0] * scale)), max(1, int(size[1] * scale))))
            target.blit(image, ((width - image.get_width()) // 2, (height - image.get_height()) // 2))
        ...
        for i, (_, choice) in enumerate(self._choices()):
            ...
            target.blit(font.render(f"{prefix}{i + 1}. {choice.get('text', '')}", True, (255, 255, 255)), (60, 60 + i * 36))
