...
class ExtensibleRuntime(CoreRuntime):
    ...
    Game-specific rules remain outside the engine. Projects register systems,
    commands and state namespaces through this facade.
    """
    ...
    def _register_builtin_extension_actions(self) -> None:
        self._extension_handlers = {
            "call_system": self._call_system,
            "emit": self._emit_action,
            "set_state": self._set_state,
            "open_scene": self._open_scene,
    ...
    def subscribe(self, event_name: str, callback, priority: int = 0):
    ...
    def register_state_namespace(self, name: str, initial=None, version: int = 1) -> None:
        self.game_state.register(name, initial, version)

    def get_state(self, path: str, default=None):
        return self.game_state.get(path, default)

    def set_state(self, path: str, value) -> None:
        self.game_state.set(path, value)

    def push_scene(self, scene, **kwargs) -> None:
        self.scene_stack.push(scene, **kwargs)

    def pop_scene(self):
        return self.scene_stack.pop()

    def replace_scene(self, scene, **kwargs) -> None:
        self.scene_stack.replace(scene, **kwargs)

    def update(self, dt: float) -> None:
        super().update(dt)
        for system in self.systems.values():
            system.update(dt, self.game_state)
        self.scene_stack.update(dt)

    def dispatch_input(self, event: object) -> bool:
        if self.scene_stack.handle_input(event):
            return True
        handled = False
        for system in self.systems.values():
            if system.handle_event(event, self.game_state):
                handled = True
        return handled
    ...
    def _call_system(self, action: Action) -> None:
        ...
        if system is None:
            raise RuntimeError(f"Unknown game system: {action.data['system']}")
        method = getattr(system, action.data["method"], None)
        if method is None or action.data["method"].startswith("_"):
            raise RuntimeError(f"System method is not callable: {action.data['method']}")
        method(*action.data.get("args", []))

    def _emit_action(self, action: Action) -> None:
        self.emit(action.data["event"], {"args": list(action.data.get("args", []))})

    def _set_state(self, action: Action) -> None:
        from vnengine.core.expressions import evaluate
        value = evaluate(action.data["expression"], self.state.variables)
        self.set_state(action.data["path"], value)
        self.emit("state.changed", {"path": action.data["path"], "value": value})

    def _open_scene(self, action: Action) -> None:
        factory = self.commands.get(f"scene:{action.data['name']}")
        if factory is None:
            raise RuntimeError(f"Scene is not registered: {action.data['name']}")
        factory(self)

    def _close_scene(self, action: Action) -> None:
        current = self.scene_stack.current
        if current is not None and getattr(current, "name", None) == action.data["name"]:
            self.pop_scene()

    def advance(self) -> None:
        if not self.state.running:
            return
        # Core Runtime owns timing/dialogue semantics. We mirror its action
        # dispatch here so extension actions can participate in the same loop.
        now = __import__("pygame").time.get_ticks() / 1000.0
        if self.state.wait_until and now < self.state.wait_until:
            return
        ...
        while self.state.index < len(self.state.story.actions) and self.state.running and not self.state.paused_for_input:
            ...
            self.state.index += 1
            self.emit("before_action", {"action": action.kind, "data": action.data})
            if self.state.conditional_stack and not all(self.state.conditional_stack) and action.kind not in ("if", "else", "endif"):
                continue
            ...
            if extension is not None:
                ...
                if handler is None:
                    raise RuntimeError(f"No runtime handler for action: {action.kind}")
                handler(action)
            self.emit("after_action", {"action": action.kind, "data": action.data})
            if action.kind in ("say", "choice", "end", "open_scene"):
                break
