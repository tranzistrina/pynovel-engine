...
class ExtensibleRuntime(CoreRuntime):
    """Compatibility runtime with a stable extension lifecycle."""
    ...
    def attach_movement(self, definition):
    ...
    def notify(self, title, body="", **kwargs):
        item = self.notifications.add(title, body, **kwargs)
    ...
    def dispatch_input(self, event):
        ...
        for system in self.systems.values():
            ...
            if handler is not None: handled = bool(handler(event, self.game_state)) or handled
        return handled
    ...
    def _register_builtin_extension_actions(self):
    ...
    def _call_system(self, action):
        ...
        method_name = str(action.data["method"])
    ...
    def save_bundle(self, path, project_version="1", metadata=None):
        ...
        bundle.metadata = dict(metadata or {})
        bundle.extensions = self.systems.serialize()
        bundle.rng = self.rng.serialize()
        bundle.save(path)
    ...
    def load_bundle(self, path, project_version="1"):
        ...
        self.state.variables = dict(bundle.state.get("runtime", {})); self.game_state.deserialize(bundle.state.get("extensions", {})); self.notifications.deserialize(bundle.state.get("notifications", {})); self.audio.deserialize(bundle.state.get("audio", {})); self.input_map = InputMap.deserialize(bundle.state.get("input_map", []))
        self.systems.deserialize(bundle.extensions)
