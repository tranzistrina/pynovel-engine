...
class MapController:
    """Input-to-event bridge for MapSurface; game rules stay external."""
    ...
    def select(self, pos: tuple[int, int]) -> Any:
        ...
        if hit is None:
            self.emit(MapEvent("map.selection_cleared", None, {}))
            return None
        event_name = "map.marker_selected" if hit.__class__.__name__ == "MapMarker" else "map.node_selected"
        self.emit(MapEvent(event_name, hit.id, {"object": hit}))
        return hit
    ...
    def move_pan(self, pos: tuple[int, int]) -> None:
        if self._dragging:
            ...
            self.emit(MapEvent("map.camera_changed", None, {"x": self.surface.camera.x, "y": self.surface.camera.y, "zoom": self.surface.camera.zoom}))
    ...
    def set_route(self, node_ids: list[str]) -> None:
        ...
        self.emit(MapEvent("map.route_changed", None, {"nodes": list(node_ids)}))
