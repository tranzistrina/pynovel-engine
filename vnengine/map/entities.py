...
class EntityRegistry:
    ...
    def remove(self, entity_id: str) -> MapEntity | None:
        return self._entities.pop(str(entity_id), None)

    def get(self, entity_id: str | EntityHandle) -> MapEntity | None:
        key = entity_id.id if isinstance(entity_id, EntityHandle) else str(entity_id)
    ...
    def handle(self, entity_id: str) -> EntityHandle:
        return self.require(entity_id).handle

    def all(self) -> tuple[MapEntity, ...]:
    ...
    def query(self, *, component: str | None = None, node_id: str | None = None) -> tuple[MapEntity, ...]:
        ...
        if component is not None:
            key = str(component)
            result = (entity for entity in result if key in entity.components)
        if node_id is not None:
            node = str(node_id)
            result = (entity for entity in result if entity.node_id == node)
        return tuple(result)

    def at_node(self, node_id: str) -> tuple[MapEntity, ...]:
    ...
    def set_component(self, entity_id: str | EntityHandle, name: str, value: Any) -> Any:
        return self.require(entity_id).set_component(name, value)

    def get_component(self, entity_id: str | EntityHandle, name: str, default: Any = None) -> Any:
        return self.require(entity_id).get_component(name, default)

    def remove_component(self, entity_id: str | EntityHandle, name: str) -> Any:
        return self.require(entity_id).remove_component(name)

    def apply_components(self, entity_id: str | EntityHandle, components: dict[str, Any], *, replace: bool = False) -> None:
        entity = self.require(entity_id)
        if replace:
            entity.components = dict(components)
        else:
            entity.components.update(components)

    def batch_set_component(self, updates: Iterable[tuple[str, str, Any]]) -> None:
        for entity_id, name, value in updates:
            self.set_component(entity_id, name, value)
    ...
    def serialize(self) -> dict[str, Any]:
        return {
            entity_id: {
                ...
                "components": dict(sorted(entity.components.items())),
    ...
    def deserialize(self, payload: dict[str, Any]) -> None:
        ...
        for entity_id, data in sorted(payload.items()):
            if not isinstance(data, dict):
                raise ValueError(f"Entity definition must be an object: {entity_id}")
            pos = data.get("position", [0, 0])
            if not isinstance(pos, (list, tuple)) or len(pos) != 2:
                raise ValueError(f"Invalid entity position: {entity_id}")
            self.add(
                MapEntity(
                    str(entity_id),
                    MapPoint(float(pos[0]), float(pos[1])),
                    data.get("node_id"),
                    dict(data.get("components", {})),
