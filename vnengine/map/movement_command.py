...
class MovementCommand:
    ...
    def execute(self, selection: MultiSelection, target_node: str, speed: float = 100.0) -> list[MovementOrder]:
        ...
        for entity_id in selection.selected:
            ...
            if route is None:
                self.emit("movement.order_unreachable", {"entity_id": entity_id, "target": target_node})
                continue
            self.movement.start(entity_id, route, speed)
            orders.append(MovementOrder(entity_id, route))
