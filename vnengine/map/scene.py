...
class MapScene:
    ...
    def __init__(self, game_map: PlayableMap, viewport: Any, *, pygame_module: Any = None, emit=None):
        ...
        self.surface = MapSurface(game_map.definition, self.viewport)
    ...
    def handle_input(self, event: Any) -> bool:
        if event.type == self.pygame.MOUSEBUTTONDOWN:
            if event.button == 2:
                self.interaction.begin_pan(event.pos); return True
            if event.button in (4, 5):
                factor = 1.1 if event.button == 4 else 1 / 1.1
                self.controller.zoom(factor, event.pos); return True
        ...
        if event.type == self.pygame.MOUSEBUTTONUP and event.button == 2:
            self.interaction.end_pan(); return True
        return False
