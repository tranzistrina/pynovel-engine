...
class ProjectDocument:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        if scenes_path.is_file(): data["scenes"] = json.loads(scenes_path.read_text(encoding="utf-8"))
        return data
    ...
    def save(self) -> None:
        ...
        if isinstance(map_data, dict):
            map_path = self.root / str(self.data.get("map_path", "map.json")); map_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def ensure_map(self) -> dict[str, Any]:
        ...
        if not isinstance(map_data, dict): raise ValueError("Project map must be an object")
        map_data.setdefault("width", 1200); map_data.setdefault("height", 700); map_data.setdefault("nodes", []); map_data.setdefault("connections", []); map_data.setdefault("entities", [])
        return map_data

    def ensure_scenes(self) -> dict[str, dict[str, Any]]:
        scenes = self.data.setdefault("scenes", {})
        if not isinstance(scenes, dict): raise ValueError("Project scenes must be an object")
        return scenes
    ...
    def add_connection(self, source: str, target: str, *, cost: float = 1.0, blocked: bool = False, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
        map_data = self.ensure_map(); node_ids = {item["id"] for item in map_data["nodes"]}
        ...
        map_data["connections"].append(connection); return connection

    def add_entity(self, entity_id: str, node_id: str, *, components: dict[str, Any] | None = None) -> dict[str, Any]:
        map_data = self.ensure_map(); node_ids = {item["id"] for item in map_data["nodes"]}
        if node_id not in node_ids: raise ValueError(f"Unknown node for entity: {node_id}")
        if self._find_by_id(map_data["entities"], entity_id): raise ValueError(f"Duplicate entity: {entity_id}")
