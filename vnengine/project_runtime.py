...
class ProjectRuntime:
    ...
    def start(self, *, transition: tuple[str, float] | None = None) -> None:
        self.switch_scene(self.project.manifest.start_scene, transition=transition); self.running = True; self.emit("runtime.started", {"scene": self.scene_id})
    ...
    def switch_scene(self, scene_id: str, *, transition: tuple[str, float] | None = None) -> Any:
        ...
        self.scene_id = scene_id; self.scene = scene; self.world = getattr(scene, "world", scene); self._call(scene, "enter"); self.emit("scene.changed", {"from": previous, "to": scene_id}); return scene
    ...
    def push_scene(self, scene_id: str, *, transition: tuple[str, float] | None = None) -> Any:
        ...
        scene = self.scenes.create(scene_id, self); self.stack.push(scene_id, scene); self.scene_id = scene_id; self.scene = scene; self.world = getattr(scene, "world", scene); self._call(scene, "enter"); self.emit("scene.pushed", {"scene": scene_id}); return scene
