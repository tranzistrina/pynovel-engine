...
class ProjectRuntime:
    ...
    def __init__(self, project: str, *, emit=None, scenes: SceneRegistry | None = None, viewport: Any = None, frontend: Any = None):
        ...
        self.components = ComponentRegistry(); self._register_builtin_components(); self.resources = ResourceRegistry(self.project.root); self._load_resources(); self.frontend = frontend
        loader = None
        ...
        self.component_systems: list[ComponentSystem] = []
    ...
    def _register_builtin_components(self) -> None:
        self.components.register("transform", defaults={"x": 0.0, "y": 0.0})
        self.components.register("state", defaults={})
        self.components.register("metadata", defaults={})

    def register_component(self, name: str, *, factory=None, requires=(), defaults=None, replace=False): return self.components.register(name, factory=factory, requires=requires, defaults=defaults, replace=replace)
    def register_component_system(self, system: ComponentSystem) -> ComponentSystem:
        ...
        self._validate_system_requirements(system); self.component_systems.append(system); return system
    def unregister_component_system(self, name: str) -> None: self.component_systems = [item for item in self.component_systems if item.name != str(name)]
    def _validate_system_requirements(self, system: ComponentSystem) -> None:
    ...
    def _load_resources(self) -> None:
        ...
        for resource_id, definition in data.items():
            ...
            try: self.resources.register(str(resource_id), str(definition["path"]), str(definition.get("type", "other")), metadata=definition.get("metadata"))
    ...
    def _create_map_scene(self, context: SceneContext) -> Any:
        ...
        game_map.world.components = context.runtime.components; game_map.world.entities.component_registry = context.runtime.components
    ...
    def evaluate(self, expression: Any) -> Any: return self.expression.evaluate(expression)

    def start(self, *, transition: tuple[str, float] | None = None) -> None:
        self.running = True; self.switch_scene(self.project.manifest.start_scene, transition=transition); self.emit("runtime.started", {"scene": self.scene_id})
    ...
    def update(self, dt: float) -> None:
        ...
        for system in tuple(self.component_systems): system.run(value, self.world.entities if self.world is not None and hasattr(self.world, "entities") else (), self.logic)
    def stop(self) -> None:
        if self.scene is not None: self._call(self.scene, "exit")
