...
class ProjectRuntime:
    ...
    def __init__(self, project: str, *, emit=None, scenes: SceneRegistry | None = None, viewport: Any = None, frontend: Any = None):
        ...
        self.resources = ResourceRegistry(self.project.root); self._load_resources()
    ...
    def _load_resources(self) -> None:
        ...
        for resource_id, definition in data.items():
            ...
            resource_path = definition.get("path"); resource_type = definition.get("type", "other")
            if resource_path is None: continue
            try: self.resources.register(str(resource_id), str(resource_path), str(resource_type), metadata=definition.get("metadata"))
    ...
    def switch_scene(self, scene_id: str, *, transition: tuple[str, float] | None = None) -> Any:
        ...
        scene = self.scenes.create(scene_id, self); self.stack.clear(); self.stack.push(scene_id, scene); self.scene_id = scene_id; self.scene = scene; self.world = getattr(scene, "world", scene); self._call(scene, "enter"); self.emit("scene.changed", {"from": previous, "to": scene_id}); return scene
