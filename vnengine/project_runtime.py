...
class ProjectRuntime:
    """Lifecycle wrapper for data-driven map projects."""
    def __init__(self, project: str, *, emit=None):
        self.project = ProjectLoader(project)
        self.emit = emit or (lambda name, data: None)
        self.world = None; self.scene_id: str | None = None; self.running = False

    def start(self) -> None:
        self.scene_id = self.project.manifest.start_scene
        if self.scene_id != "map": raise ValueError(f"Unsupported start scene: {self.scene_id}")
        self.world = self.project.load_map(emit=self.emit); self.running = True
        self.emit("runtime.started", {"scene": self.scene_id})

    def update(self, dt: float) -> None:
        if self.running and self.world is not None: self.world.update(max(0.0, float(dt)))
    ...
    def load_state(self, state: dict[str, Any]) -> None:
        ...
        self.running = True; self.emit("runtime.loaded", {"scene": scene})
