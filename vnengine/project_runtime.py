...
class ProjectRuntime:
    """Runtime for data-driven games with shared state, scenes, resources and transitions."""
    ...
    def __init__(self, project: str, *, emit=None, scenes: SceneRegistry | None = None, viewport: Any = None, frontend: Any = None):
        ...
        self.resources = ResourceRegistry(self.project.root)
        self._load_resources()
        self.frontend = frontend
        loader = None
        if frontend is not None and getattr(frontend, "_pygame", None) is not None:
            from .pygame_assets import PygameAssetLoader
            loader = PygameAssetLoader(frontend._pygame)
        self.assets = AssetRuntime(self.resources, loader=loader)
    ...
    def _load_resources(self) -> None:
        ...
        if not isinstance(data, dict):
            return
        for resource_id, definition in data.items():
            if not isinstance(definition, dict) or definition.get("path") is None:
                continue
            try:
                self.resources.register(str(resource_id), str(definition["path"]), str(definition.get("type", "other")), metadata=definition.get("metadata"))
            except ValueError:
                continue

    def _register_project_scenes(self) -> None:
        try:
            definitions = self.project.load_scenes()
        except (OSError, json.JSONDecodeError):
            return
        for scene_id, definition in definitions.items():
            if isinstance(scene_id, str) and isinstance(definition, dict) and not self.scenes.has(scene_id):
    ...
    def _create_map_scene(self, context: SceneContext) -> Any:
        ...
        game_map = context.runtime.project.load_map(emit=context.runtime.emit)
        ...
        if viewport is None:
            raise RuntimeError("Map scene requires a viewport")
    ...
    def set(self, key: str, value: Any) -> Any:
        return self.logic.set(str(key), value)

    def change(self, key: str, amount: Any = 1) -> Any:
        return self.logic.change(str(key), amount)

    def evaluate(self, expression: Any) -> Any:
        return self.expression.evaluate(expression)

    def start(self, *, transition: tuple[str, float] | None = None) -> None:
        self.running = True
        self.switch_scene(self.project.manifest.start_scene, transition=transition)
    ...
    def switch_scene(self, scene_id: str, *, transition: tuple[str, float] | None = None) -> Any:
        ...
        if transition:
            self.transitions.start(*transition)
        if self.scene is not None:
            self._call(self.scene, "exit")
        scene = self.scenes.create(scene_id, self)
        self.stack.clear()
        self.stack.push(scene_id, scene)
    ...
    def push_scene(self, scene_id: str, *, transition: tuple[str, float] | None = None) -> Any:
        ...
        try:
            ...
            if self.stack.current is not None:
                self.scene_id = self.stack.current_id
                self.scene = self.stack.current
                self.world = getattr(self.scene, "world", self.scene)
                self._call(self.scene, "resume")
            raise
        self.emit("scene.pushed", {"scene": scene_id})
        return scene

    def pop_scene(self, *, transition: tuple[str, float] | None = None) -> Any:
        if len(self.stack) <= 1:
            raise IndexError("Cannot pop the root scene")
        if transition:
            self.transitions.start(*transition)
        current = self.scene
        if current is not None:
            self._call(current, "exit")
        self.stack.pop()
        self.scene_id = self.stack.current_id
        self.scene = self.stack.current
        self.world = getattr(self.scene, "world", self.scene)
        self._call(self.scene, "resume")
        self.emit("scene.popped", {"scene": self.scene_id})
        return self.scene
    ...
    def render(self, target: Any) -> None:
        ...
        if callable(renderer):
            ...
            drawer = getattr(self.scene, "draw", None)
            if callable(drawer):
                drawer(target)
    ...
    def update(self, dt: float) -> None:
        if not self.running or self.scene is None:
            return
        self.transitions.update(max(0.0, float(dt)))
        update = getattr(self.scene, "update", None)
        if callable(update):
            update(max(0.0, float(dt)))
    ...
    def save_state(self) -> dict[str, Any]:
        ...
        world = saver() if callable(saver) else (self.world.serialize() if self.world is not None else None)
        return {"scene_stack": list(self.stack.ids()), "scene": self.scene_id, "logic": self.logic.serialize(), "world": world}

    def load_state(self, state: dict[str, Any]) -> None:
        ids = state.get("scene_stack") or [state.get("scene", self.project.manifest.start_scene)]
        if not isinstance(ids, list) or not ids:
            raise ValueError("save state must contain a non-empty scene stack")
        self.switch_scene(str(ids[0]))
        for scene_id in ids[1:]:
            self.push_scene(str(scene_id))
        self.logic.deserialize(state.get("logic", {}))
        self.expression = ExpressionEvaluator(self.logic.state)
        if state.get("world") is not None and self.world is not None:
            self.world.deserialize(state["world"])
        refresh = getattr(self.scene, "refresh", None)
        if callable(refresh):
            refresh()
        self.running = True
        self.emit("runtime.loaded", {"scene": self.scene_id})
