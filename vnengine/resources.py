...
class Resource:
    ...
    def to_dict(self) -> dict[str, Any]: return {"id": self.id, "type": self.type, "path": self.path, "metadata": dict(self.metadata)}


class ResourceRegistry:
    """Deterministic project resource catalog with safe paths and lazy caching."""
    def __init__(self, root: str | Path):
        self.root = Path(root).resolve(); self._resources: dict[str, Resource] = {}; self._cache: dict[str, Any] = {}
    def register(self, resource_id: str, path: str | Path, resource_type: str, *, metadata: dict[str, Any] | None = None, replace: bool = False) -> Resource:
        ...
        self._safe_path(path); resource = Resource(key, str(resource_type), str(path).replace("\\", "/"), dict(metadata or {})); self._resources[key] = resource; self._cache.pop(key, None); return resource
    def unregister(self, resource_id: str) -> None: self._resources.pop(resource_id, None); self._cache.pop(resource_id, None)
    ...
    def path(self, resource_id: str) -> Path: return self._safe_path(self.get(resource_id).path)
    ...
    def inspect(self) -> dict[str, Any]: return {"root": str(self.root), "count": len(self._resources), "resources": [{**r.to_dict(), "exists": self.exists(r.id)} for r in self._resources.values()]}
    ...
    def _safe_path(self, path: str | Path) -> Path:
        ...
        except ValueError as exc: raise ValueError(f"Resource path escapes project root: {path}") from exc
        return candidate
