...
class Game:
    def __init__(self, project: str | Path):
        ...
        lang = "ru"; project_cfg = self.project / "project.json"
        if project_cfg.exists():
            try:
                import json
                cfg = json.loads(project_cfg.read_text(encoding="utf-8")); lang = cfg.get("default_language", lang)
            except (OSError, ValueError): pass
        self.catalog = Catalog(self.project / "locales", lang); self.menu = GameMenu(self.project, self.catalog)
    def _toggle_fullscreen(self):
        s=self.runtime.state; s.settings["fullscreen"]=not s.settings.get("fullscreen",False); self.screen=pygame.display.set_mode((0,0),pygame.FULLSCREEN) if s.settings["fullscreen"] else pygame.display.set_mode((1280,720),pygame.RESIZABLE)
    ...
    def _handle_menu_result(self,result):
        ...
        elif result.action=="new_game":self.runtime.new_game()
    ...
    def run(self):
        ...
        self.screen=pygame.display.set_mode((1280,720),pygame.RESIZABLE); pygame.display.set_caption(self.runtime.state.story.title); clock=pygame.time.Clock(); self.runtime.apply_scene_manifest(); self.runtime.advance()
        while self.runtime.state.running:
            for event in pygame.event.get():
                if event.type==pygame.QUIT:self.runtime.state.running=False
                    ...
                    if self.menu.is_open:self._handle_menu_result(self.menu.handle_mouse(event.pos,self.screen.get_size()))
                    else:
                        s=self.runtime.state
                        if s.choice_options:
            ...
            s=self.runtime.state; self.runtime.update(max(0.0,clock.get_time()/1000.0)); speed=max(1.0,float(self.menu.settings.get("text_speed",42))); s.settings["text_speed"]=speed; s.settings["volume"]=float(self.menu.settings.get("volume",0.8))
