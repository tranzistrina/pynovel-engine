...
class Game:
    def __init__(self, project: str | Path):
        self.project=Path(project); lang="ru"
        ...
        self.menu=GameMenu(self.project,self.catalog); self.menu.settings["text_speed"]=self.profile.text_speed; self.menu.settings["volume"]=self.profile.volume
    ...
    def _toggle_fullscreen(self):
        s=self.runtime.state; s.settings["fullscreen"]=not s.settings.get("fullscreen",False); self.profile.fullscreen=s.settings["fullscreen"]; self._save_profile()
    ...
    def run(self):
        ...
        while self.runtime.state.running:
            ...
            if not self.at_title:
                s=self.runtime.state; self.runtime.update(max(0.0,clock.get_time()/1000.0)); s.settings["text_speed"]=float(self.menu.settings.get("text_speed",42)); s.settings["volume"]=float(self.menu.settings.get("volume",0.8))
