...
class RuntimeFacade:
    ...
    def __init__(self, runtime: Any, *, fixed_timestep: float | None = None, max_steps: int = 8, profiling: bool = False) -> None:
    ...
    def start(self, **kwargs: Any) -> None:
        ...
        if self.clock is not None: self.clock.reset()

    def step(self, dt: float, *, events: list[Any] | tuple[Any, ...] = (), target: Any = None, record: bool = True) -> dict[str, Any]:
        if record: self.replay.record(dt, events)
        handled = 0
        if self.clock is None:
            self._step_once(float(dt), events, target); handled = self._last_handled
        else:
            for _ in range(self.clock.advance(float(dt))):
                self._step_once(self.clock.step, events, target); handled += self._last_handled
        return {"handled_events": handled, "running": bool(self.runtime.running), "state": self.runtime.save_state(), "profile": self.profiler.snapshot()}

    def _step_once(self, dt: float, events: list[Any] | tuple[Any, ...], target: Any) -> None:
        handled = 0
        with self.profiler.measure("input"):
            for event in events:
                if self.runtime.handle_input(event): handled += 1
        self._last_handled = handled
        with self.profiler.measure("update"): self.runtime.update(max(0.0, dt))
        with self.profiler.measure("render"): self.runtime.render(target)

    def play_replay(self, replay: ReplaySession | None = None, *, target: Any = None) -> list[dict[str, Any]]:
        player = ReplayPlayer(replay or self.replay); results = []
    ...
    def snapshot(self) -> dict[str, Any]: return self.runtime.save_state()
    def restore(self, state: dict[str, Any]) -> None: self.runtime.load_state(dict(state))
    def stop(self) -> None: self.runtime.stop()
    def reset_replay(self) -> None: self.replay.clear()
    ...
    def feature_state(self) -> dict[str, bool]:
        """Report which optional features are currently enabled."""
        return {"fixed_timestep": self.clock is not None, "profiling": self.profiler.enabled, "replay_recording": True}
