...
class SystemRegistry:
    ...
    def enabled_specs(self, phase: str | None = None) -> tuple[SystemSpec, ...]:
        ...
        return tuple(spec for spec in specs if spec.enabled and (phase is None or phase in spec.phases))
    def instantiate(self, name: str) -> Any:
        spec = self.get(name); factory = self._factories.get(spec.name)
        return factory(spec) if factory is not None else None
    def validate_definitions(self) -> list[str]:
        errors: list[str] = []; names = set(self._specs)
        for name, spec in sorted(self._specs.items()):
            invalid_phases = [phase for phase in spec.phases if phase not in self.VALID_PHASES]
            if invalid_phases: errors.append(f"System {name} references invalid phases: {invalid_phases}")
            for requirement in spec.requires:
                if not requirement: errors.append(f"System {name} has an empty component requirement")
    ...
    def _ordered_specs(self) -> list[SystemSpec]:
        specs = dict(self._specs); indegree = {name: 0 for name in specs}; edges: dict[str, set[str]] = {name: set() for name in specs}
