...
class GameMenu:
    def __init__(self, project: Path, catalog=None):
        ...
        self.mode = "closed"
        self.selected = 0
        self.options = ["Resume", "Save", "Load", "History", "Settings", "Main Menu", "Quit"]
        self.slots = list(range(1, 6))
        self.settings = {"text_speed": 42, "volume": 0.8}
        self._font = self._small = None
        self.load_settings()

    def _fonts(self):
        if self._font is None:
            self._font = pygame.font.Font(None, 36)
            self._small = pygame.font.Font(None, 26)
    ...
    def label(self, key: str, fallback: str) -> str:
        return self.catalog.get(key, fallback) if self.catalog else fallback
    ...
    def open(self):
        self.mode, self.selected = "main", 0

    def close(self):
        self.mode = "closed"

    def _set_mode(self, mode: str):
        self.mode, self.selected = mode, 0

    def handle_key(self, key: int) -> MenuResult | None:
        if self.mode == "closed":
            if key == pygame.K_ESCAPE:
                self.open(); return MenuResult("open")
            return None
        if key == pygame.K_ESCAPE:
        ...
        if self.mode == "main" and key == pygame.K_s:
            self.slot_action = "save"; self._set_mode("save_slots"); return None
        if self.mode == "main" and key == pygame.K_l:
            self.slot_action = "load"; self._set_mode("load_slots"); return None
        if self.mode == "main" and key == pygame.K_h:
            self._set_mode("history"); return None
        if self.mode == "main" and key == pygame.K_o:
            self._set_mode("settings"); return None
        if self.mode in ("save_slots", "load_slots") and pygame.K_1 <= key <= pygame.K_5:
            self.selected = key - pygame.K_1; return self.activate()
        if self.mode == "settings" and self.selected == 0 and key in (pygame.K_LEFT, pygame.K_RIGHT):
            self.settings["text_speed"] = max(8, min(120, self.settings["text_speed"] + (8 if key == pygame.K_RIGHT else -8))); self.save_settings(); return None
        if self.mode == "settings" and self.selected == 1 and key in (pygame.K_LEFT, pygame.K_RIGHT):
            self.settings["volume"] = max(0.0, min(1.0, round(self.settings["volume"] + (0.1 if key == pygame.K_RIGHT else -0.1), 1))); self.save_settings(); return None
        if self.mode == "settings" and self.selected == 2 and key in (pygame.K_LEFT, pygame.K_RIGHT): return MenuResult("fullscreen")
        return None

    def _count(self):
        return {"main": len(self.options), "save_slots": len(self.slots) + 1, "load_slots": len(self.slots) + 1, "history": 1, "settings": 3}.get(self.mode, 1)

    def activate(self) -> MenuResult:
        if self.mode == "main":
            action = ["resume", "save_menu", "load_menu", "history", "settings", "main_menu", "quit"][self.selected]
        ...
        if self.mode == "history":
            self._set_mode("main"); return MenuResult("noop")
        if self.mode == "settings" and self.selected == 2: return MenuResult("fullscreen")
        return MenuResult("noop")

    def handle_mouse(self, pos, size) -> MenuResult | None:
        if self.mode == "closed": return None
        w, h = size; px, py = pos; panelx=max(60,w*0.18); panelw=min(w-120,w*0.64); top=max(50,h*0.08)
        if self.mode == "main":
    ...
    def draw(self, screen: pygame.Surface, history: list[tuple[str, str]]):
        ...
        panel=pygame.Rect(max(60,w*0.18),max(50,h*0.08),min(w-120,w*0.64),min(h-100,h*0.84)); pygame.draw.rect(screen,(18,22,34),panel,border_radius=18); pygame.draw.rect(screen,(220,220,230),panel,2,border_radius=18)
        ...
        if self.mode == "main":
            ...
            for speaker,text in history[-12:]:
                screen.blit(self._small.render(((speaker+": ") if speaker else "")+text[:100],True,(235,235,242)),(panel.x+24,y)); y+=34
